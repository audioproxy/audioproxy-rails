# blob-resolution Specification

## Purpose
How an ActiveStorage object becomes a source string the proxy can resolve: which inputs `url_for` unwraps to a blob, what each storage service maps to, which keys are refused rather than turned into a plausible-but-wrong path, and what happens on a service this gem cannot express.
## Requirements
