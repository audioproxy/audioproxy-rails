...
- **THEN** the proxy answers 200 with audio, rather than 403

#### Scenario: The signature is load-bearing
- **WHEN** a single byte of a correctly signed URL's path is altered without re-signing
- **THEN** the proxy answers 403, proving the previous scenario asserted something
...
- **THEN** the proxy answers 410, not 403 and not 422
